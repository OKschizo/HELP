...
function ManageContent({ params }: { params: { address: string } }) {
  ...
  const [shareOnExplore, setShareOnExplore] = useState<boolean>(false)
  ...
  useEffect(() => { (async () => {
    try {
      ...
      if (col) {
        ...
        setShareOnExplore(Boolean((col as any).shareOnExplore))
      }
    } catch {}
  })() }, [drop])
      ...
      <Card className="space-y-4 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <label className="flex items-center gap-3 p-3 rounded-xl bg-[var(--bg-elevated)] border border-[var(--stroke-soft)] cursor-pointer select-none">
              <input type="checkbox" checked={shareOnExplore} onChange={e => setShareOnExplore(e.target.checked)} className="w-4 h-4" />
              <div>
                <div className="text-sm font-medium">Share this collection on Explore</div>
                <div className="text-xs text-[var(--fg-muted)]">When enabled, your collection will appear on the Explore page once deployed and with a header image.</div>
              </div>
            </label>
          </div>
        </div>
